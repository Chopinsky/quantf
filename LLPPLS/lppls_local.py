...
class LPPLS(object):
    ...
    def pow(self, val, p):
        return np.power(np.abs(val), p)

    def _fi(self, tc, m, time_series):
        return [self.pow((tc - t), m) for t in time_series]

    def _gi(self, tc, m, w, time_series):
        if self.use_ln:
            return [self.pow((tc - t), m) * np.cos(w * self.log(tc - t)) for t in time_series]
        else:
            return [self.pow((tc - t), m) * np.cos(w * (tc - t)) for t in time_series]

    def _hi(self, tc, m, w, time_series):
        if self.use_ln:
            return [self.pow((tc - t), m) * np.sin(w * self.log(tc - t)) for t in time_series]
        else:
            return [self.pow((tc - t), m) * np.sin(w * (tc - t)) for t in time_series]
    ...
    def lppls(self, t, tc, m, w, a, b, c1, c2):
        if self.use_ln:
            return a + self.pow(tc - t, m) * (
                        b + ((c1 * np.cos(w * self.log(tc - t))) + (c2 * np.sin(w * self.log(tc - t)))))
        else:
            return a + self.pow(tc - t, m) * (b + ((c1 * np.cos(w * (tc - t))) + (c2 * np.sin(w * (tc - t)))))
    ...
    def _func_compute_indicator(self, args):
        ...
        for j in range(n_fits):
            ...
            w_in_range = 2 < w < 25
            n_oscillation = ((w / 2) * self.log(abs((tc - first) / (last - first)))) > 2.5
            # for bubble end flag
